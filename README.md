...
| Cache      | Redis (ioredis) — result cache + Pub/Sub bridge |
| WebSocket  | `ws` library — room-based broadcast             |
| Validation | Zod — API layer + LLM response layer            |
| Upload     | Multer — PDF/TXT file handling                  |
...
| `marksPerQuestion` | number | ✅ | 1–100 |
| `questionTypes` | JSON array | ✅ | `["mcq","short","long","true_false"]` |
| `difficultyDistribution[easy]` | number | ✅ | must sum to totalQuestions |
| `difficultyDistribution[medium]` | number | ✅ | |
| `difficultyDistribution[hard]` | number | ✅ | |
| `dueDate` | ISO string | ❌ | |
...
|---|---|---|
| `connected` | Server → Client | `{ event, assignmentId, message }` |
| `job.progress` | Server → Client | `{ event, assignmentId, message, progress }` |
| `job.complete` | Server → Client | `{ event, assignmentId, progress: 100, data: GeneratedPaper }` |
| `job.error` | Server → Client | `{ event, assignmentId, message }` |
